"""
LongMemEval500 recall evaluation using SimpleMem default single-view retrieval path.

This script follows the core flow in test_longmemeval500.py:
...
from test_longmemeval500 import load_longmemeval_dataset
...
def main():
    parser = argparse.ArgumentParser(description="LongMemEval500 single-view recall eval with llm_spans back-mapping")
    ...
    parser.add_argument("--result-file", type=str, default="longmemeval500_singleview_results.json")
    ...
    for sample_idx, sample in enumerate(samples):
        ...
        if args.parallel_questions and len(sample.qa) > 1:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = {
                    executor.submit(process_single_qa, system, raw_store, sample.sample_id, qa_idx, qa): qa_idx
            ...
            for qa_idx, qa in enumerate(sample.qa):
                result, recall = process_single_qa(system, raw_store, sample.sample_id, qa_idx, qa)
                if not result:
                    continue
                result["build_time"] = build_time
                qa_results.append(result)
    ...
    print("=" * 80)
    print("LongMemEval500 Single-View Recall Summary")
    print("=" * 80)
