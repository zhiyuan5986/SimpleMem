#!/usr/bin/env python3
"""Batch consolidate LongMemEval extraction traces with LongLLMLingua+LAQuer span mining.

For each ``longmemeval500_sample_{idx}_extraction_trace.json`` found under a directory, this
script will:
1) run the same coarse-to-fine filtering flow as
   ``scripts/filter_extraction_trace_longllmlingua_laquer.py``;
2) auto-locate the corresponding sample DB directory (``longmemeval500_sample_{idx}``);
...
SAMPLE_TRACE_RE = re.compile(r"longmemeval500_sample_(\d+)_extraction_trace\.json$")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Auto consolidate LongMemEval traces and span DBs for all samples.")
    parser.add_argument("--logs-dir", type=Path, required=True, help="Directory containing longmemeval500 sample trace logs.")
    parser.add_argument(
        "--db-search-roots",
        type=Path,
        nargs="+",
        default=[Path(".")],
        help="One or more roots used to auto-find per-sample DB dirs named longmemeval500_sample_{idx}.",
    )
    parser.add_argument(
        "--output-suffix",
        type=str,
        default="longllmlingua_filtered",
        help="Suffix for per-sample output JSON: longmemeval500_sample_{idx}_{suffix}.json",
    )
    ...
    parser.add_argument(
        "--fallback-spans-root",
        type=Path,
        default=Path("outputs/longmemeval500_spans_fallback"),
        help="Fallback root if sample DB dir is not found; uses <root>/longmemeval500_sample_{idx}.",
    )
...
def find_trace_files(logs_dir: Path) -> list[tuple[int, Path]]:
    ...
    for path in logs_dir.glob("longmemeval500_sample_*_extraction_trace.json"):
...
def find_sample_db_dir(sample_idx: int, search_roots: list[Path]) -> Path | None:
    target_name = f"longmemeval500_sample_{sample_idx}"
...
def resolve_original_dia_id(turn: dict[str, Any]) -> Any:
    """Resolve LongMemEval's original per-turn dia_id from turn metadata when available."""
...
def process_single_sample(
    *,
    ...
    output_json = trace_json.with_name(f"longmemeval500_sample_{sample_idx}_{args.output_suffix}.json")

    db_dir = find_sample_db_dir(sample_idx, args.db_search_roots)
    used_fallback_db = False
    if db_dir is None:
        db_dir = args.fallback_spans_root / f"longmemeval500_sample_{sample_idx}"
...
def main() -> None:
    ...
    for sample_idx, trace_json in traces:
        # if sample_idx > 0:
        #     break
        print(f"[LongMemEval] processing sample={sample_idx} trace={trace_json}")
        summary.append(
